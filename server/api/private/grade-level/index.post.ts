...
export default defineEventHandler(async (event) => {
  ...
  const [existingGradeLevel] = await db.select().from(gradeLevel).where(eq(gradeLevel.grade_level_name, grade_level_name));
  ...
  const [createdGradeLevel] = await db.insert(gradeLevel).values({
    grade_level_name,
  }).$returningId();
