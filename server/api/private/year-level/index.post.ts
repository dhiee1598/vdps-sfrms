...
import { yearLevel, yearLevelInsertSchema } from '~~/server/db/schema/year-level-schema';
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readValidatedBody(event, yearLevelInsertSchema.safeParse);
  ...
  const [existingYearLevel] = await db.select().from(yearLevel).where(eq(yearLevel.name, name));
  ...
  const [createdYearLevel] = await db.insert(yearLevel).values({
    name,
  }).$returningId();
