...
export default defineEventHandler(async (event) => {
  ...
  const [existingSection] = await db.select().from(sections).where(and(eq(sections.section_name, body.data.section_name), ne(sections.id, id)));
  ...
  return { success: true, data: result, message: 'Section updated successfully' };
});
