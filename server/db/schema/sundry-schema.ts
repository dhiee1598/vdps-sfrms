...
const sundries = mysqlTable('sundries', {
  ...
  sundry_amount: decimal('sundry_amount', { precision: 10, scale: 2 }).notNull(),
  createdAt: timestamp('created_at').defaultNow(),
});

const sundryInsertSchema = createInsertSchema(sundries, {
  sundry_name: schema => schema.min(3), // string
  sundry_description: schema => schema.optional(), // int
  sundry_amount: () => z.coerce.string().regex(/^\d+(\.\d{1,2})?$/, 'Invalid amount'),
}).omit({
  id: true,
  createdAt: true,
});
